...
// void pcm2g711(short *lBuf,unsigned char *oBuf,int nSmp)  
VOID G711AlawEncode(SHORT *lBuf, BYTE *oBuf, LONG nSmp)
{
    int i;
    for(i=0;i<nSmp;i++)
    {
	*(oBuf+i)= alaw_comp_table[((*(lBuf+i))>>2)&0x3fff];
    }
    // return ;
}

// void g7112pcm(unsigned char *iBuf,short *lBuf,int nSmp)
VOID G711AlawDecode(BYTE *iBuf, SHORT *lBuf, LONG nSmp)
{
    int i;
    for(i=0;i<nSmp;i++)
    {
	*(lBuf+i)= alaw_exp_table[*(iBuf+i)];
    }
    // return ;
}
...
VOID G711UlawEncode(SHORT *lBuf, BYTE *oBuf, LONG nSmp)
{
    ...
    for(i=0;i<nSmp;i++)
    {
	*(oBuf+i)= ulaw_comp_table[((*(lBuf+i))>>2)&0x3fff];
    }
}
