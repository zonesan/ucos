...
static Word16 wg_flag, v_flag, less_count;

extern Word16 MeanLSF[M];
extern Word16 Min_buffer[16];
extern Word16 Prev_Min, Next_Min, Min;
extern Word16 MeanE, MeanSE, MeanSLE, MeanSZC;
extern Word16 prev_energy;
extern Word16 count_sil, count_update, count_ext;
extern Word16 wg_flag, v_flag, less_count;
*/
/* local function 
         ...
         Word16 *marker)
{
	  ...
	  ZC = 0;
	  for (i=ZC_START+1; i<=ZC_END; i++)//vad560
	    	if (mult(sigpp[i-1], sigpp[i]) < 0)
	      		ZC = add(ZC, 410);     

	  
	  //if(sub(frm_count, 129) < 0){//vad620
	  if((frm_count < 129)){//vad620
	  //vad624
	    	//if (sub(ENERGY, Min) < 0){
	    	if (ENERGY < Min){
	      		Min = ENERGY;
	  ...
	  if((frm_count & 0x0007) == 0){	//vad1300
	    	Prev_Min = Min_buffer[0];
	    	for (i=1; i<16; i++){ //vad1332
  	...
  	//if(sub(frm_count, 129) >= 0){//vad660
  	if((frm_count >= 129)){
		//vad660
	    	if(((frm_count & 0x0007) ^ (0x0001)) == 0){//vad661
	      		Min = Prev_Min;
	      		Next_Min = MAX_16;
	    	}
	    	//if (sub(ENERGY, Min) < 0)//
	    	if (ENERGY < Min)//vad700
	      		Min = ENERGY;
	    	//if (sub(ENERGY, Next_Min) < 0)
	    	if (ENERGY <  Next_Min)//vad712
	      		Next_Min = ENERGY;
	    
	    	if((frm_count & 0x0007) == 0){//vad728
	      		for (i=0; i<15; i++)//vad752
  	...
  	if ((frm_count <= INIT_FRAME) )//vad880
  	//vad880
  	...
  	if ((frm_count >= INIT_FRAME)){//
    		//if (sub(frm_count, INIT_FRAME) == 0){//
    		if ((frm_count == INIT_FRAME)){//vad932
    			//vad3100
     			...
     			MeanSZC = extract_h(acc0);
      			...
      			MeanSE = sub(MeanE, 2048);   
      			MeanSLE = sub(MeanE, 2458);  
    		}
		//vad940
		...
		//vad1020
    		if(wg_flag == 1){//vad1024
    		//vad2544
      			if((pprev_marker == VOICE) && (prev_marker == VOICE) && 
         				(*marker == NOISE) && (sub(abs_s(sub(prev_energy,ENERGY)), 614) <= 0)){
        			count_ext++;
        			*marker = VOICE;
        			v_flag = 1;
    		...
    		if(*marker == NOISE)//vad1044
    		//vad1048
      			count_sil++;

    		if((*marker == VOICE) && ((count_sil > 10)) && //vad1076
       		(sub(sub(ENERGY,prev_energy), 614) <= 0)){//vad2760
       		
      			*marker = NOISE;
      			count_sil=0;
    		}

    		if(*marker == VOICE)//vad1088
      			count_sil=0;//vad2680

    		if ((sub(sub(ENERGY, 614), MeanSE) < 0) && ((frm_count > 128))
        		&& (!v_flag) && ((rc < 19661)))//vad1096
        		//vad2712
      			*marker = NOISE;
		//vad1096
    		if ((sub(sub(ENERGY,614),MeanSE) < 0) && ((rc < 24576))
        			&& ((SD < 83))){ 
        			//vad2812
     			count_update++;
      			//if (sub(count_update, INIT_COUNT) < 0){//vad3124
      			if ((count_update < INIT_COUNT)){//vad3388
			        COEF = 24576;
			        C_COEF = 8192;
			        COEFZC = 26214;
			        C_COEFZC = 6554;
      			...
      			//else if (sub(count_update, INIT_COUNT+10) < 0){
        		...
        		//else  if (sub(count_update, INIT_COUNT+20) < 0){
          		...
          		//else if (sub(count_update, INIT_COUNT+30) < 0){
            		...
            		//else   if (sub(count_update, INIT_COUNT+40) < 0){
		...
		//vad1316
    		//if((sub(frm_count, 128) > 0) && (((sub(MeanSE,Min) < 0) &&
               ...
               if(((frm_count > 128)) && ((((MeanSE < Min)) &&
                        ((SD < 83))) || (sub(MeanSE,Min) > 2048))){
      			MeanSE = Min;
      			count_update = 0;
    		}
  	}
               ...
               )
{
  ...
  acc0 = L_mult(dSZC, -14680);          
  acc0 = L_mac(acc0, 8192, -28521);    
  acc0 = L_shr(acc0, 8);                
  acc0 = L_add(acc0, L_deposit_h(SD));
  if (acc0 > 0) return(VOICE);

  acc0 = L_mult(dSZC, 19065);           
  acc0 = L_mac(acc0, 8192, -19446);     
  acc0 = L_shr(acc0, 7);               
  acc0 = L_add(acc0, L_deposit_h(SD));
  if (acc0 > 0) return(VOICE);

  
  acc0 = L_mult(dSZC, 20480);          
  acc0 = L_mac(acc0, 8192, 16384);      
  acc0 = L_shr(acc0, 2);               
  acc0 = L_add(acc0, L_deposit_h(dSE));
  if (acc0 < 0) return(VOICE);

  acc0 = L_mult(dSZC, -16384);          
  acc0 = L_mac(acc0, 8192, 19660);     
  acc0 = L_shr(acc0, 2);                
  acc0 = L_add(acc0, L_deposit_h(dSE));
  if (acc0 < 0) return(VOICE);
 
  acc0 = L_mult(dSE, 32767);           
  acc0 = L_mac(acc0, 1024, 30802);      
  if (acc0 < 0) return(VOICE);
  
 
  acc0 = L_mult(SD, -28160);           
  acc0 = L_mac(acc0, 64, 19988);       
  acc0 = L_mac(acc0, dSE, 512);        
  if (acc0 < 0) return(VOICE);

  acc0 = L_mult(SD, 32767);            
  acc0 = L_mac(acc0, 32, -30199);      
  if (acc0 > 0) return(VOICE);


  acc0 = L_mult(dSZC, -20480);         
  acc0 = L_mac(acc0, 8192, 22938);     
  acc0 = L_shr(acc0, 2);               
  acc0 = L_add(acc0, L_deposit_h(dSE));
  if (acc0 < 0) return(VOICE);

  acc0 = L_mult(dSZC, 23831);          
  acc0 = L_mac(acc0, 4096, 31576);      
  acc0 = L_shr(acc0, 2);               
  acc0 = L_add(acc0, L_deposit_h(dSE));
  if (acc0 < 0) return(VOICE);
  
  acc0 = L_mult(dSE, 32767);           
  acc0 = L_mac(acc0, 2048, 17367);     
  if (acc0 < 0) return(VOICE);
  
 
  acc0 = L_mult(SD, -22400);            
  acc0 = L_mac(acc0, 32, 25395);       
  acc0 = L_mac(acc0, dSLE, 256);       
  if (acc0 < 0) return(VOICE);
  
 
  acc0 = L_mult(dSE, -30427);           
  acc0 = L_mac(acc0, 256, -29959);     
  acc0 = L_add(acc0, L_deposit_h(dSLE));
  if (acc0 > 0) return(VOICE);

  acc0 = L_mult(dSE, -23406);          
  acc0 = L_mac(acc0, 512, 28087);       
  acc0 = L_add(acc0, L_deposit_h(dSLE));
  if (acc0 < 0) return(VOICE);

  acc0 = L_mult(dSE, 24576);            
  acc0 = L_mac(acc0, 1024, 29491);      
  acc0 = L_mac(acc0, dSLE, 16384);      
  if (acc0 < 0) return(VOICE);
